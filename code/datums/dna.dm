...
/datum/dna/proc/generate_unique_features()
	...
	/// SKYRAPTOR WARNING: we need to update all uses of mutant dna features to follow the new setup
	...
	for(var/blocknum in 1 to DNA_FEATURE_BLOCKS)
		. += L[blocknum] || random_string(GET_UF_BLOCK_LEN(blocknum), GLOB.hex_characters) /// SKYRAPTOR EDIT: upstream goofed for years on this
