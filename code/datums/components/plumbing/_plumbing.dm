...
/datum/component/plumbing/proc/create_overlays(atom/movable/parent_movable, list/overlays)
	...
	for(var/direction in GLOB.cardinals)
		...
		var/image/overlay
		if(turn_connects)
			overlay = image(connection_icon, "[direction_text]-[ducting_layer]", layer = duct_layer) /// SKYRAPTOR EDIT: custom icon
		else
			overlay = image(connection_icon, "[direction_text]-[ducting_layer]-s", layer = duct_layer) /// SKYRAPTOR EDIT: custom icon
		...
		if(extend_pipe_to_edge && !extension_handled)
			var/image/edge_overlay = image(connection_icon, "edge-extension", layer = duct_layer) /// SKYRAPTOR EDIT: custom icon
