...
/datum/bodypart_overlay/mutant/get_overlay(layer, obj/item/bodypart/limb)
	...
	var/mob/living/carbon/human/the_humie = limb.owner
	if(sprite_datum.color_src == SPRITE_ACC_SCRIPTED_COLOR && !isnull(the_humie))
		draw_color = sprite_datum.color_override(the_humie)
	var/base_r = ..()
	var/list/overlays = list()
	overlays += base_r
	if(!isnull(sprite_datum))
