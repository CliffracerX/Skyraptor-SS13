...
/// SKYRAPTOR ADDITION BEGIN - SKYRAPTOR WARNING: we might need largescale rewrites for goonstam
...
/mob/living/carbon/update_stamina_hud(shown_stamina_loss)
	...
	/// SKYRAPTOR ADDITION BEGIN: sanity checking
	if(!hud_used?.stamina_capmod || !hud_used?.stamina_regmod || !hud_used?.stamina_stamcrit || !hud_used?.stamina_alerts)
		return
	...
	//var/stam_crit_threshold = maxHealth - crit_threshold /// SKYRAPTOR REMOVAL
...
/mob/living/carbon/fully_heal(heal_flags = HEAL_ALL)
	...
	exit_stamina_stun() /// SKYRAPTOR ADDITION: always exit stamstun
	return ..()
